...
public class SqliteSessionTemplateTesting {
    private SessionTemplate template = null;

    @Before
    public void init() throws ContextException {
        if (template == null) {
            XmlAppContext context = new XmlAppContext(SessionFactoryBuilder.class.getResourceAsStream("/sqlite-template-mimosa.xml"));
    ...
    public void testCalculate() {
        AutoResult object = template.calculate(Criteria.fun(TableUser.class)
                .addFunction(BasicFunction.COUNT, TableUser.age)
                .childGroupBy(TableUser.userName)
                .gt(TableUser.id, 10));
        System.out.println(object.getSingle());
    }
