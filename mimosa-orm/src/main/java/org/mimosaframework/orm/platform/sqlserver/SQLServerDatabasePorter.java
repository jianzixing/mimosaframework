...
public class SQLServerDatabasePorter extends AbstractDatabasePorter {
    ...
    protected void transformationSQLAssistField(SelectBuilder builder, SQLBuilder sqlBuilder, Map<Class, String> aliasNames, Map<Class, MappingTable> mappingTables) {
        ...
        /**
         * 如果有group by 则分页查询的字段不能直接使用over() as
         */
        List<Object> restricts = builder.getRestrict();
        if (restricts != null) {
            for (Object restrict : restricts) {
                if (restrict instanceof GroupBuilder) {
                    ...
                    aliasName = aliasNames.get(groupTableClass);
                    MappingTable mappingTable = mappingTables.get(groupTableClass);
                    List<MappingField> mappingFields = mappingTable.getMappingPrimaryKeyFields();
                    if (mappingFields == null) mappingField = mappingTable.getMappingFields().iterator().next();
                    else mappingField = mappingFields.get(0);
                }
            }
        }

        if (mappingField == null) {
            Map.Entry<Class, MappingTable> entry = mappingTables.entrySet().iterator().next();
            Class tableClass = entry.getKey();
            MappingTable mappingTable = entry.getValue();
            List<MappingField> mappingFields = mappingTable.getMappingPrimaryKeyFields();
            if (mappingFields == null) mappingField = mappingTable.getMappingFields().iterator().next();
            else mappingField = mappingFields.get(0);
            aliasName = aliasNames.get(tableClass);
        }

        sqlBuilder.addSplit();
        sqlBuilder.addString("ROW_NUMBER() over(ORDER BY [" + aliasName + "].[" + mappingField.getMappingColumnName() + "] ASC)")
                .AS().addWrapString("RowNo");
    }
}
