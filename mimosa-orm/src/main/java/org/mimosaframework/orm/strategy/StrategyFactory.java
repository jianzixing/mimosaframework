...
public class StrategyFactory {
    ...
    public static Serializable applyStrategy(Configuration values,
                                             ...
                                             ModelObject source,
                                             Session session,
                                             boolean replace) throws StrategyException {
        return applyStrategy(values, table, object, source, session, null, replace);
    }
    ...
    public static Serializable applyStrategy(Configuration values,
                                             ...
                                             Session session,
                                             boolean replace) throws StrategyException {
        return applyStrategy(values, table, object, null, session, null, replace);
    }
    ...
    public static Serializable applyStrategy(Configuration values,
        ...
        if (object != null && fields != null && fields.size() > 0) {
            for (MappingField f : fields) {
                Object value = object.get(f.getMappingFieldName());
                if (value != null && replace == false) {
                    continue;
                }
