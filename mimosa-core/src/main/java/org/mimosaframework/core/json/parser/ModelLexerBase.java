...
import static org.mimosaframework.core.json.parser.ModelToken.COLON;
import static org.mimosaframework.core.json.parser.ModelToken.COMMA;
...
import static org.mimosaframework.core.json.parser.ModelToken.LPAREN;
import static org.mimosaframework.core.json.parser.ModelToken.RBRACE;
import static org.mimosaframework.core.json.parser.ModelToken.RBRACKET;
import static org.mimosaframework.core.json.parser.ModelToken.RPAREN;
 ...
 */
public abstract class ModelLexerBase implements ModelLexer, Closeable {
    ...
    protected TimeZone                       timeZone           = Model.defaultTimeZone;
    protected Locale                         locale             = Model.defaultLocale;
    ...
    public ModelLexerBase(int features){
            ...
            switch (ch) {
                case '\'':
                    if (!isEnabled(Feature.AllowSingleQuotes)) {
                        throw new ModelException("Feature.AllowSingleQuotes is false");
                    }
                ...
                default:
                    if (isEOF()) { // JLS
                        if (token == EOF) {
                            throw new ModelException("EOF error");
                        }
    ...
    public final void nextToken(int expect) {
        ...
        for (;;) {
            switch (expect) {
                case ModelToken.LBRACE:
                    if (ch == '{') {
                        token = ModelToken.LBRACE;
                        next();
                        return;
                    }
                    if (ch == '[') {
                        token = ModelToken.LBRACKET;
                        next();
                        return;
                    }
                    break;
                case ModelToken.COMMA:
                    if (ch == ',') {
                        token = ModelToken.COMMA;
                        next();
                        return;
                    }

                    if (ch == '}') {
                        token = ModelToken.RBRACE;
                        next();
                        return;
                    }

                    if (ch == ']') {
                        token = ModelToken.RBRACKET;
                        next();
                        return;
                    }

                    if (ch == EOI) {
                        token = ModelToken.EOF;
                        return;
                    }
                    break;
                case ModelToken.LITERAL_INT:
                    if (ch >= '0' && ch <= '9') {
                        pos = bp;
                        scanNumber();
                        return;
                    ...
                    break;
                case ModelToken.LITERAL_STRING:
                    if (ch == '"') {
                        pos = bp;
                        scanString();
                        return;
    ...
    public final void nextTokenWithChar(char expect) {
        ...
        for (;;) {
            ...
            throw new ModelException("not match " + expect + " - " + ch);
        }
    }
    ...
    protected void skipComment() {
        ...
        if (ch == '/') {
            ...
            throw new ModelException("invalid comment");
        }
    }
    ...
    public final String scanSymbol(final SymbolTable symbolTable, final char quote) {
        ...
        for (;;) {
            ...
            if (chLocal == EOI) {
                throw new ModelException("unclosed.str");
            }
            ...
            if (chLocal == '\\') {
                ...
                switch (chLocal) {
                    ...
                    default:
                        this.ch = chLocal;
                        throw new ModelException("unclosed.str.lit");
                }
                continue;
            }
    ...
    public final String scanSymbolUnQuoted(final SymbolTable symbolTable) {
        ...
        if (!firstFlag) {
            throw new ModelException("illegal identifier : " + ch //
                                    + info());
        }
    ...
    public final void scanString() {
        ...
        for (;;) {
            ...
            if (ch == EOI) {
                ...
                throw new ModelException("unclosed string : " + ch);
            }
        ...
        token = ModelToken.LITERAL_STRING;
        this.ch = next();
    }
    ...
    protected final static char[] typeFieldName = ("\"" + Model.DEFAULT_TYPE_KEY + "\":\"").toCharArray();
    ...
    public Collection<String> scanFieldStringArray(char[] fieldName, Class<?> type) {
        ...
        for (;;) {
            ...
            if (chLocal == '"') {
                ...
                throw new ModelException("illega str");
            }
    ...
    public final void scanTrue() {
        if (ch != 't') {
            throw new ModelException("error parse true");
        }
        next();
        ...
        if (ch == ' ' || ch == ',' || ch == '}' || ch == ']' || ch == '\n' || ch == '\r' || ch == '\t' || ch == EOI
            || ch == '\f' || ch == '\b' || ch == ':') {
            token = ModelToken.TRUE;
        } else {
            throw new ModelException("scan true error");
        }
    }

    public final void scanNullOrNew() {
        if (ch != 'n') {
            throw new ModelException("error parse null or new");
        }
        next();
        ...
        if (ch == 'u') {
            next();
            if (ch != 'l') {
                throw new ModelException("error parse null");
            }
            next();
            ...
            if (ch == ' ' || ch == ',' || ch == '}' || ch == ']' || ch == '\n' || ch == '\r' || ch == '\t' || ch == EOI
                ...
                throw new ModelException("scan null error");
            }
            return;
        }

        if (ch != 'e') {
            throw new ModelException("error parse new");
        }
        next();
        ...
        if (ch == ' ' || ch == ',' || ch == '}' || ch == ']' || ch == '\n' || ch == '\r' || ch == '\t' || ch == EOI
            || ch == '\f' || ch == '\b') {
            token = ModelToken.NEW;
        } else {
            throw new ModelException("scan new error");
        }
    }

    public final void scanFalse() {
        if (ch != 'f') {
            throw new ModelException("error parse false");
        }
        next();
        ...
        if (ch == ' ' || ch == ',' || ch == '}' || ch == ']' || ch == '\n' || ch == '\r' || ch == '\t' || ch == EOI
            || ch == '\f' || ch == '\b' || ch == ':') {
            token = ModelToken.FALSE;
        } else {
            throw new ModelException("scan false error");
        }
    }
    ...
    public final void scanIdent() {
        ...
        for (;;) {
            ...
            if ("null".equalsIgnoreCase(ident)) {
                ...
                token = ModelToken.UNDEFINED;
            } else if ("Set".equals(ident)) {
                token = ModelToken.SET;
            } else if ("TreeSet".equals(ident)) {
                token = ModelToken.TREE_SET;
            } else {
                token = ModelToken.IDENTIFIER;
            }
            return;
    ...
    private void scanStringSingleQuote() {
        ...
        for (;;) {
            ...
            if (chLocal == EOI) {
                ...
                throw new ModelException("unclosed single-quote string");
            }
    ...
    public final void scanNumber() {
        ...
        if (isDouble) {
            token = ModelToken.LITERAL_FLOAT;
        } else {
            token = ModelToken.LITERAL_INT;
        }
    }
    ...
    public final Number decimalValue(boolean decimal) {
        ...
        try {
            ...
            throw new ModelException(ex.getMessage() + ", " + info());
        }
    }
