...
public class MiscCodec implements ObjectSerializer, ObjectDeserializer {
    ...
    public void write(ModelSerializer serializer, Object object, Object fieldName, Type fieldType,
        ...
        if (objClass == SimpleDateFormat.class) {
        ...
        } else if (object instanceof ModelStreamAware) {
            ModelStreamAware aware = (ModelStreamAware) object;
            ...
            throw new ModelException("not support class : " + objClass);
        }

        out.writeString(strVal);
    }

    protected void writeIterator(ModelSerializer serializer, SerializeWriter out, Iterator<?> it) {
        int i = 0;
        out.write('[');
        while (it.hasNext()) {
            if (i != 0) {
    ...
    public <T> T deserialze(DefaultModelParser parser, Type clazz, Object fieldName) {
        ...
        if (clazz == InetSocketAddress.class) {
            ...
            parser.accept(ModelToken.LBRACE);

            InetAddress address = null;
            int port = 0;
            for (;;) {
                String key = lexer.stringVal();
                lexer.nextToken(ModelToken.COLON);
                ...
                if (key.equals("address")) {
                    ...
                    if (lexer.token() != ModelToken.LITERAL_INT) {
                        throw new ModelException("port is not int");
                    }
                    port = lexer.intValue();
                    lexer.nextToken();
                } else {
        ...
        if (objVal == null) {
            ...
            throw new ModelException("expect string");
        }
        ...
        if (clazz == URL.class) {
            try {
                ...
                throw new ModelException("create url error", e);
            }
        }
        ...
        if (clazz == InetAddress.class || clazz == Inet4Address.class || clazz == Inet6Address.class) {
            try {
                ...
                throw new ModelException("deserialize inet adress error", e);
            }
        }
        ...
        if (clazz == ModelPath.class) {
            return (T) new ModelPath(strVal);
        }
        ...
        if (className.equals("java.nio.file.Path")) {
            try {
                ...
                throw new ModelException("Path deserialize erorr");
            } catch (NoSuchMethodException ex) {
                method_paths_get_error = true;
            } catch (IllegalAccessException ex) {
                throw new ModelException("Path deserialize erorr", ex);
            } catch (InvocationTargetException ex) {
                throw new ModelException("Path deserialize erorr", ex);
            }
        }

        throw new ModelException("MiscCodec not support " + className);
    }
