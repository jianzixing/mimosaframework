...
public class ASMSerializerFactory implements Opcodes {
    ...
    static final String            JSONSerializer           = ASMUtils.type(ModelSerializer.class);
    ...
    public JavaBeanSerializer createJavaBeanSerializer(SerializeBeanInfo beanInfo) throws Exception {
        ...
        if (clazz.isPrimitive()) {
            throw new ModelException("unsupportd class " + clazz.getName());
        }
        ...
        MethodVisitor mw = new MethodWriter(cw, ACC_PUBLIC, "<init>", "(" + ASMUtils.desc(SerializeBeanInfo.class) + ")V", null, null);
        mw.visitVarInsn(ALOAD, 0);
        mw.visitVarInsn(ALOAD, 1);
        mw.visitMethodInsn(INVOKESPECIAL, JavaBeanSerializer, "<init>", "(" + ASMUtils.desc(SerializeBeanInfo.class) + ")V");
        ...
        for (int i = 0; i < getters.length; ++i) {
            ...
            if (fieldInfo.method != null) {
                mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(desc(fieldInfo.declaringClass)));
                mw.visitLdcInsn(fieldInfo.method.getName());
                mw.visitMethodInsn(INVOKESTATIC, ASMUtils.type(ASMUtils.class), "getMethodType",
            ...
            if (fieldClass == byte.class //
                ...
                if (elementClass == String.class //
                    ...
                    if (elementClass != null && Modifier.isPublic(elementClass.getModifiers())) {
                        ...
                        mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc(elementClass)));
                        ...
                        if (context.writeDirect) {
                            ...
                            mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc(elementClass))); // fieldType
                    ...
                    if (elementClass != null && Modifier.isPublic(elementClass.getModifiers())) {
                        mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc((Class<?>) elementType)));
                ...
                if (context.writeDirect && Modifier.isPublic(fieldClass.getModifiers())) {
                    ...
                    mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc(fieldClass))); // fieldType
                    mw.visitLdcInsn(fieldInfo.serialzeFeatures); // features
                    mw.visitMethodInsn(INVOKEVIRTUAL, JavaBeanSerializer, "writeAsArrayNonContext", //
                                       "(L" + JSONSerializer + ";Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/reflect/Type;I)V");
                    mw.visitJumpInsn(GOTO, instanceOfEnd_);
    ...
    private void _enum(Class<?> clazz, MethodVisitor mw, FieldInfo fieldInfo, Context context) {
       ...
       if (context.writeDirect) {
            ...
            mw.visitVarInsn(ALOAD, context.var("enum"));
            mw.visitVarInsn(ALOAD, Context.fieldName);
            mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc((Class<?>) fieldInfo.fieldClass)));
            mw.visitLdcInsn(fieldInfo.serialzeFeatures);
            mw.visitMethodInsn(INVOKEVIRTUAL, JSONSerializer, "writeWithFieldName",
                    "(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/reflect/Type;I)V");
        }
    ...
    private void _get(MethodVisitor mw, Context context, FieldInfo fieldInfo) {
        ...
        if (method != null) {
            ...
            mw.visitMethodInsn(INVOKEVIRTUAL, ASMUtils.type(declaringClass), method.getName(), ASMUtils.desc(method));
            ...
            mw.visitFieldInsn(GETFIELD, ASMUtils.type(fieldInfo.declaringClass), field.getName(),
        ...
        if (format != null) {
            ...
            if (fieldInfo.fieldType instanceof Class<?> //
                ...
                if (fieldInfo.fieldClass == String.class) {
                    mw.visitLdcInsn(org.mimosaframework.core.json.asm.Type.getType(ASMUtils.desc(String.class)));
                } else {
                    mw.visitVarInsn(ALOAD, 0);
                    mw.visitFieldInsn(GETFIELD, context.className, fieldInfo.name + "_asm_fieldType",
                                      "Ljava/lang/reflect/Type;");
    ...
    private void _processValue(MethodVisitor mw, FieldInfo fieldInfo, Context context, Label _end) {
        ...
        mw.visitMethodInsn(INVOKEVIRTUAL, JavaBeanSerializer, "getBeanContext", "(I)" + ASMUtils.desc(BeanContext.class));
        mw.visitVarInsn(ALOAD, Context.obj);
        mw.visitVarInsn(ALOAD, Context.fieldName);
