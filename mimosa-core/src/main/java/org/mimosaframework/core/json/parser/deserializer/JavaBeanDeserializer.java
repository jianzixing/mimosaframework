...
public class JavaBeanDeserializer implements ObjectDeserializer {
    ...
    public Object createInstance(DefaultModelParser parser, Type type) {
        if (type instanceof Class) {
            if (clazz.isInterface()) {
                Class<?> clazz = (Class<?>) type;
                ClassLoader loader = Thread.currentThread().getContextClassLoader();
                final ModelObject obj = new ModelObject();
        ...
        try {
            ...
            throw new ModelException("create instance error, class " + clazz.getName(), e);
        }
    ...
    public <T> T deserialze(DefaultModelParser parser, Type type, Object fieldName, int features) {
        return deserialze(parser, type, fieldName, null, features);
    }

    @SuppressWarnings({"unchecked"})
    public <T> T deserialzeArrayMapping(DefaultModelParser parser, Type type, Object fieldName, Object object) {
        final ModelLexer lexer = parser.lexer; // xxx
        ...
        for (int i = 0, size = sortedFieldDeserializers.length; i < size; ++i) {
            ...
            if (fieldClass == int.class) {
                ...
                lexer.nextToken(ModelToken.LBRACKET);
                Object value = parser.parseObject(fieldDeser.fieldInfo.fieldType);
                fieldDeser.setValue(object, value);

                check(lexer, seperator == ']' ? ModelToken.RBRACKET : ModelToken.COMMA);
    ...
    protected void check(final ModelLexer lexer, int token) {
        if (lexer.token() != token) {
            throw new ModelException("syntax error");
        }
    }

    protected Enum<?> scanEnum(ModelLexer lexer, char seperator) {
        throw new ModelException("illegal enum. " + lexer.info());
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    protected <T> T deserialze(DefaultModelParser parser, //
                               Type type, // 
                               Object fieldName, // 
                               Object object, //
                               int features) {
        if (type == Model.class || type == ModelObject.class) {
            return (T) parser.parse();
        }

        final ModelLexerBase lexer = (ModelLexerBase) parser.lexer; // xxx
        ...
        try {
            ...
            if (token != ModelToken.LBRACE && token != ModelToken.COMMA) {
                ...
                throw new ModelException(buf.toString());
            }

            if (parser.resolveStatus == DefaultModelParser.TypeNameRedirect) {
                parser.resolveStatus = DefaultModelParser.NONE;
            }
            ...
            for (int fieldIndex = 0; ; fieldIndex++) {
                ...
                if (fieldDeser != null) {
                    ...
                    if (fieldClass == int.class || fieldClass == Integer.class) {
                        ...
                        if (lexer.matchStat > 0) {
                            matchField = true;
                            valueParsed = true;
                        } else if (lexer.matchStat == ModelLexer.NOT_MATCH_NAME) {
                            continue;
                        }
                        ...
                        if (token == ModelToken.LITERAL_STRING) {
                            ...
                            if ("@".equals(ref)) {
                                ...
                                if (parentContext.object != null) {
                                    ...
                                    parser.addResolveTask(new DefaultModelParser.ResolveTask(parentContext, ref));
                                    parser.resolveStatus = DefaultModelParser.NeedToResolve;
                                }
                            } else if ("$".equals(ref)) {
                                ...
                                if (rootContext.object != null) {
                                    ...
                                    parser.addResolveTask(new DefaultModelParser.ResolveTask(rootContext, ref));
                                    parser.resolveStatus = DefaultModelParser.NeedToResolve;
                                }
                            } else {
                            ...
                            throw new ModelException("illegal ref, " + ModelToken.name(token));
                        }
                        ...
                        if (lexer.token() == ModelToken.LITERAL_STRING) {
                            ...
                            if (deserizer == null) {
                                ...
                                if (expectClass == null ||
                                    ...
                                    throw new ModelException("type not match");
                                }
                            }
                ...
                if (matchField) {
                    if (!valueParsed) {
                        ...
                        if (lexer.matchStat == ModelLexer.END) {
                            break;
                        }
                    }
                } else {
                ...
                if (lexer.token() == ModelToken.IDENTIFIER || lexer.token() == ModelToken.ERROR) {
                    throw new ModelException("syntax error, unexpect token " + ModelToken.name(lexer.token()));
                }
            }
            ...
            if (object == null) {
                ...
                if (beanInfo.creatorConstructor != null) {
                    try {
                        ...
                        throw new ModelException("create instance error, "
                    ...
                    try {
                        ...
                        throw new ModelException("create factory method error, " + beanInfo.factoryMethod.toString(), e);
                    }
                }
            }
            ...
            try {
                ...
                throw new ModelException("build object error", e);
            }
    ...
    public boolean parseField(DefaultModelParser parser, String key, Object object, Type objectType,
                              Map<String, Object> fieldValues) {
        ModelLexer lexer = parser.lexer; // xxx
    ...
    public Object createInstance(Map<Object, Object> map, ParserConfig config) //
    ...
    protected Object parseRest(DefaultModelParser parser, Type type, Object fieldName, Object instance, int features) {
    ...
    protected static void parseArray(Collection collection, //
                                     ...
                                     DefaultModelParser parser, //
                                     Type type, //
                                     Object fieldName) {

        final ModelLexerBase lexer = (ModelLexerBase) parser.lexer;
        int token = lexer.token();
        if (token == ModelToken.NULL) {
            lexer.nextToken(ModelToken.COMMA);
            token = lexer.token();
        ...
        if (ch == '[') {
            lexer.next();
            lexer.setToken(ModelToken.LBRACKET);
        } else {
            lexer.nextToken(ModelToken.LBRACKET);
        }
        ...
        if (ch == ',') {
            lexer.next();
            lexer.setToken(ModelToken.COMMA);
        } else {
            lexer.nextToken(ModelToken.COMMA);
        }
